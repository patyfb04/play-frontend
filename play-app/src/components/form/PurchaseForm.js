...
export default class PurchaseForm extends Component {
  ...
  connection = new HubConnectionBuilder()
    .withUrl(`${window.TRADING_SERVICE_URL}/messageHub`, {
  ...
  async purchaseItem() {
    let confirmPurchase = window.confirm(
      `Purchase ${this.state.quantity} ${this.state.name} for ${
  ...
  async fetchRetry(idempotencyId, tries) {
    ...
    return fetch(`${window.PURCHASE_API_URL}`, {
      ...
      .then(async (response) => {
        ...
        console.log(
          "Purchase request completed with status " + response.status
        );
      ...
      .catch((err) => {
        ...
        return setTimeout(
          () => this.fetchRetry(idempotencyId, triesLeft),
          5000
        );
      });
  }
